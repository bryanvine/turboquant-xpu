...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # Initialize TurboQuant buffers (Pi, signs, centroids) if TQ cache
        if kv_cache_dtype.startswith("turboquant_"):
            self._init_turboquant_buffers(kv_cache_dtype, self.head_size,
                                          prefix)
    ...
    def _init_turboquant_buffers(
        ...
        self.register_buffer(
            "_tq_signs",
            generate_wht_signs(head_size, seed),
            persistent=False,
        )
        centroids, _ = solve_lloyd_max(head_size, tq_config.centroid_bits)
        self.register_buffer(
            "_tq_centroids",
            centroids,
            persistent=False,
