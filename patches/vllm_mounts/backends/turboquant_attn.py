...
# XPU FlashAttention only supports head_dim <= 256, but Gemma4 has
# head_dim=512 for global attention layers. Disable FA for TQ prefill
# on XPU and use SDPA fallback instead.
...
class TurboQuantAttentionBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        head_size here is the effective_head_size (= slot_size_aligned // 2)
        passed from the KV cache spec. We undo the // 2 to get the actual
        slot_size_aligned for the cache shape.
        """
        # head_size = slot_size_aligned // 2 (set in get_kv_cache_spec)
        # Undo to get slot_size_aligned for the cache tensor shape
        slot_size_aligned = head_size * 2
        return (num_blocks, block_size, num_kv_heads, slot_size_aligned)
